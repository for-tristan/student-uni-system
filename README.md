...
The dataset is generated by `scripts/generate_data.py` and lives in `data/`.
It is fully **synthetic** — no real student information is used. Generation
is deterministic (fixed random seed) so the same data is produced on every run.

| File               | Rows | Description                                              |
|--------------------|------|----------------------------------------------------------|
| `students.csv`     | 25   | Student profiles: year, major, interests, skills        |
| `courses.csv`      | 40   | Course catalog across 8 categories with prerequisites   |
| `enrollments.csv`  | 149  | Student-course enrollments with status and grade        |

### Course categories

AI · Data Science · Web · Cybersecurity · Software Engineering ·
Mathematics · Mobile · Systems · Theory
...
The validation suite checks column schemas, ID uniqueness, referential
integrity (enrollments → students & courses), prerequisite validity
(no dangling references, no self-prerequisites), enrollment-status
enums, and grade ranges.
